...
class AccountsController extends \Main\Controller {
    ...
    function profile($id, $name) {
		...
		$this->doc->setFacebookMetaData("og:url", DOMAIN. url("AccountsController@profile", ["id" => $data['account_id'], "name" => sanitize($data['account_name']['firstname']."-".$data['account_name']['lastname']) ] ));
