								...
								for($i=0; $i<count($data['articles']); $i++) {
									$html[] = "<div class='col-lg-3 col-md-6 col-sm-12 col-12'>";
										$html[] = "<div class='card mb-3' title='".$data['articles'][$i]['title']."'>";
											$html[] = "<div class='p-image img-responsive img-responsive-21x9 card-img-top bg-blue' style='height:150px; background-image: url(".$data['articles'][$i]['banner'].");'></div>";
											$html[] = "<div class='card-body mb-0 pb-2'>";
												$html[] = "<div class='p-description' style='height:70px;'>";
													$html[] = "<h3 class='p-title card-title mb-1' title='".$data['articles'][$i]['title']."'>".nicetrim($data['articles'][$i]['title'], 35)."</h3>";
													$html[] = "<p class='fs-12'>".nicetrim(strip_tags($data['articles'][$i]['content']), 45)."</p>";
												$html[] = "</div>";
											$html[] = "</div>";
									...
									$html[] = "<div class='list-group list-group-flush'>";
										if($data['categories']) {
											for($i=0; $i<count($data['categories']); $i++) {
												$html[] = "<a class='list-group-item d-flex justify-content-between text-decoration-none' href='".url("ArticlesController@index", null, ["category" => strtolower($data['categories'][$i]['category'])])."'>";
